...
export function normalizeCategory(cat, desc) {
  ...
  const mapping = {
    '식비': '생활비', '식료품': '생활비', '식자재': '생활비', '정육': '생활비', '식육': '생활비', '쇼핑': '생활비', '카페': '생활비', '커피': '생활비', '편의점': '생활비', '식생활': '생활비', '외식': '생활비', '배달': '생활비', '마트': '생활비', '식재료': '생활비', '생필품': '생활비', '생활용품': '생활비',
    ...
    '기타': '기타', '고정비': '기타', '금융': '기타', '수수료': '기타', '구독료/서비스': '기타', '서비스': '기타', '반려동물': '의료/건강', '펫': '의료/건강', '동물병원': '의료/건강', '잡비': '기타', '가족': '가족', '공금': '기타', '경조사': '기타', '경조사비': '기타', '회비': '기타', '구독': '기타', '구독료': '기타', '세금': '기타', '과태료': '기타',
  ...
  // 허용 목록 밖의 분류는 내용 기반 추론으로 교정한다.
  // (버그수정) 과거엔 분류가 '비어 있을 때만' 추론해서, Gemini가 '편의점'·'카페'처럼
  // 목록에 없는 값을 주면 매핑에도 안 걸리고 그대로 시트에 기록됐다.
  // 예: '세븐일레븐 신도림태영점'이 분류 '편의점'으로 기록 → 생활비 집계에서 누락.
  // 추론 힌트에 원래 분류 문자열도 함께 넣는다('편의점' 자체가 생활비 키워드에 걸리도록).
  const needsInference = !DEFAULT_CATEGORIES.includes(resolvedCat);
  if (needsInference) {
    const hint = `${desc || ''} ${trimmed}`;
    const descUpper = hint.toUpperCase();
    const keywordRules = [
      { cat: '생활비', keywords: ['국밥', '식당', '카페', '커피', '베이커리', '제과', '치킨', '피자', '스타벅스', '배달의민족', '요기요', '배민', '코스트코', '이마트', '홈플러스', '롯데마트', '쿠팡', '마켓컬리', '다이소', '편의점', '세븐일레븐', '7-ELEVEN', '7ELEVEN', '세븐일레', '이마트24', '미니스톱', '스토리웨이', '노브랜드', '트레이더스', 'CU', 'GS25', '지에스네트웍스', '(주)지에스네트웍스', '식비', '외식', '배달', '마트', '식재료', '생활용품', '생필품', '올리브영', '푸드', '반찬', '밀키트', '정육', '식육', '청과', '수산'] },
      ...
      { cat: '투자/저축', keywords: ['적금', '주식', '예금', '청약', '펀드', '개인연금', '저축', '투자', '증권'] },
    ...
    // 추론에도 실패하면 '기타'로 떨어뜨린다 — 목록 밖 값을 시트에 남기지 않는다.
    resolvedCat = inferred || '기타';
  }
