...
const SheetsAPI = (() => {
  ...
  function _normalizeCategory(cat, desc) {
    ...
    // 허용 목록 밖의 분류는 내용 기반 추론으로 교정한다(worker/src/sheets.js 와 동일 규칙).
    // (버그수정) 과거엔 분류가 '비어 있을 때만' 추론해서, '편의점'처럼 목록에 없는 값이
    // 매핑에도 안 걸리고 그대로 남아 생활비 집계에서 누락됐다.
    const needsInference = !cfg.DEFAULT_CATEGORIES.includes(resolvedCat);
    if (needsInference) {
      const descUpper = `${desc || ''} ${trimmed}`.toUpperCase();
      const keywordRules = [
      ...
      // 추론에도 실패하면 '기타'로 떨어뜨린다 — 목록 밖 값을 남기지 않는다.
